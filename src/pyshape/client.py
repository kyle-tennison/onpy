...
class Client:
    ...
    def get_document(self, id: str|None = None, name: str|None = None) -> Document:
        ...
        if id is None and name is None:
            raise PyshapeParameterError("A name or id must be provided to fetch a document")
        ...
        if name:
            ...
            if name_matches > 1:
                raise PyshapeParameterError(f"Multiple documents with name {name}. Use document id instead")
        ...
        if model_candidate is None:
            raise PyshapeParameterError("Unable to find a document with " + (f"name {name}" if name else f"id {id}"))
